...
def count_unique_pairs(df: pd.DataFrame, blocks: Dict[str, List[int]]) -> int:
    """Return the number of *unique* cross-table candidate pairs implied by blocks.
    We de-duplicate pairs that co-occur in multiple blocks by using a set.
    """
    seen = set()
    total = 0
...
def candidate_pairs(df: pd.DataFrame, blocks: Dict[str, List[int]], total: Optional[int] = None) -> List[Tuple[int, int]]:
    seen = set()
    pairs: List[Tuple[int, int]] = []
    bar = tqdm(total=total, desc="Generating candidate pairs", file=sys.stdout, unit="pair", miniters=1, dynamic_ncols=True, leave=True) if total is not None else tqdm(desc="Generating candidate pairs", unit="pair")
    for k, idxs in blocks.items():
        if len(idxs) < 2:
            continue
        by_tbl = {}
...
def score_pairs(df: pd.DataFrame, pairs: List[Tuple[int,int]], auto_thr: float, review_thr: float, show_left_progress: bool = False, pairs_total: Optional[int] = None) -> List[MatchDecision]:
    decisions = []
    # Optional: track how many LEFT rows have been attempted at least once
    left_seen = set()
    left_bar = tqdm(total=df[df['__table']==0].shape[0], desc='Left rows attempted', unit='row',
                    file=sys.stdout, miniters=1, dynamic_ncols=True, leave=True) if show_left_progress else None
    # Accurate progress: one tick per pairwise comparison actually scored
    for i, j in tqdm(pairs, total=(pairs_total if pairs_total is not None else (len(pairs) if isinstance(pairs, (list, tuple)) else None)), file=sys.stdout,
                     desc='Matching pairs', unit='pair'):
        a, b = df.loc[i], df.loc[j]
        if left_bar is not None and a['__table'] == 0 and i not in left_seen:
            left_seen.add(i)
            try:
                left_bar.update(1)
            except Exception:
    ...
    try:
        left_bar.close() if left_bar is not None else None
    except Exception:
        pass
    return decisions
...
def link_entities(tables: Dict[str, pd.DataFrame], auto_thr: float = AUTO_THR, review_thr: float = REVIEW_THR, progress_mode: str = 'blocked'):
    frames = []
    for name, df in tables.items():
        frames.append(normalise_table(df, name))
    df_all = pd.concat(frames, ignore_index=True)

    # Build blocks and count unique candidate pairs
    blocks = make_blocks(df_all)
    total_pairs = count_unique_pairs(df_all, blocks)

    # Choose how to enumerate pairs
    if progress_mode == 'cartesian':
        left_idx = df_all[df_all['__table'] == 0].index.tolist()
        right_idx = df_all[df_all['__table'] == 1].index.tolist()
        pairs_total = len(left_idx) * len(right_idx)
        ...
        pairs = candidate_pairs(df_all, blocks, total=total_pairs)
        pairs_total = total_pairs

    # Score and cluster
    decisions = score_pairs(df_all, pairs, auto_thr, review_thr, show_left_progress=True, pairs_total=pairs_total)
...
def match_entities(
    ...
    progress_mode: str = 'blocked') -> Tuple[List[Tuple[int, int]], pd.DataFrame]:
    ...
    out = link_entities({"left": left_std, "right": right_std},
                        auto_thr=auto_threshold, review_thr=review_threshold, progress_mode=progress_mode)
...
if __name__ == "__main__":
    ...
    pairs, review = match_entities(t1, t2, schema_left, schema_right, review_path=None)
