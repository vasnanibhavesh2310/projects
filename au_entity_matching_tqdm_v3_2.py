...
def _schema_apply(df: pd.DataFrame, schema: Dict[str, str]) -> pd.DataFrame:
    ...
    def col_or_blank(key: str) -> pd.Series:
        ...
        if col is not None and col in df.columns:
            try:
                ...
                # if casting fails, coerce to string via map
                return df[col].map(lambda x: "" if pd.isna(x) else str(x))
        # missing in schema or not present in df -> blank series
    ...
    # derived normals (safe on blanks)
    out["email_n"]    = out["email"].map(_norm_email)
    out["phone_n"]    = out["phone"].map(_norm_phone_au)
    out["dob_n"]      = out["dob"].map(_parse_dmy_any)
    out["name_key"]   = (out["first_name"].astype(str) + " " + out["last_name"].astype(str)).map(
        lambda s: re.sub(r"[^a-z]+"," ", (s or "").lower()).strip()
    )
    return out
