...
def score_pairs(df: pd.DataFrame, pairs: List[Tuple[int,int]], auto_thr: float, review_thr: float) -> List[MatchDecision]:
    decisions = []
    # Accurate progress: one tick per pairwise comparison actually scored
    for i, j in tqdm(pairs, total=len(pairs) if isinstance(pairs, (list, tuple)) else None,
...
def link_entities(tables: Dict[str, pd.DataFrame], auto_thr: float = AUTO_THR, review_thr: float = REVIEW_THR):
    frames = []
    ...
    decisions = score_pairs(df_all, pairs, auto_thr, review_thr)
...
def match_entities(
    ...
    out = link_entities({"left": left_std, "right": right_std},
                        auto_thr=auto_threshold, review_thr=review_threshold)
